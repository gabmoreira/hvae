...
def poincare_distance(x1, x2):
    eps = 0.0
    d   = np.arccosh(1.0 + (2.0 * np.square(np.linalg.norm(x1 - x2) )) /
                    ( (1.0-np.square(np.linalg.norm(x1))) * (1.0-np.square(np.linalg.norm(x2))) + eps) ) 
    return d
